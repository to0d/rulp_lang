package beta.test.math;

import org.junit.jupiter.api.Test;

import alpha.rulp.utils.RulpTestBase;

class XRFactorArithmeticTest extends RulpTestBase {

	@Test
	void test_arithmetic_1_add_1() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_1_add_2_str() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_1_add_3_class() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_2_sub() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_3_by_1() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_3_by_2() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_4_div_1() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_4_div_2() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_5_mod_1() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_5_mod_2() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_6_power_1() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_6_power_2() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_7_and() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_8_or() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_9_xor() {
		_setup();
		_run_script();
	}

	@Test
	void test_arithmetic_a_not() {
		_setup();
		_run_script();
	}
}
